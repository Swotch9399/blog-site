...
export default function EditPostPage({ params }: EditPostPageProps) {
  ...
  useEffect(() => {
    const getUser = async () => {
      ...
      if (user) {
        setUser(user)
        await fetchPost(params.id, user.id)
      } else {
  ...
  const fetchPost = async (postId: string, userId: string) => {
    try {
      ...
      if (error) {
        setMessage("Yazı bulunamadı veya erişim yetkiniz yok")
      ...
      setMessage("Yazı yüklenirken hata oluştu")
    } finally {
      ...
      <header className="border-b bg-white/80 backdrop-blur-sm">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <h1 className="text-xl font-semibold">Yazı Düzenle</h1>
          </div>
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Yazınızı güncelleyin ve değişiklikleri kaydedin</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            <div className="p-4 bg-gray-50 rounded-lg">
              <p className="text-sm text-gray-600">
                Mevcut durum:
                <Badge variant={formData.published ? "default" : "secondary"} className="ml-2">
